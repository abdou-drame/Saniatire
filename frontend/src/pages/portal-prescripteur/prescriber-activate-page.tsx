...
export function PrescriberActivatePage() {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const { data } = await prescriberApi.post("/portail-prescripteur/activer", {
        token,
        password,
    ...
    onSuccess: () => {
      navigate("/portail-prescripteur/login", {
        replace: true,
        state: { notice: "Compte activé. Vous pouvez vous connecter." },
      });
    },
    onError: (error) => {
      if (axios.isAxiosError(error) && error.response?.status === 422) {
        setNotice("Ce lien d'activation n'est plus valide ou a déjà été utilisé.");
        return;
      }
      ...
      <div className="flex min-h-screen items-center justify-center bg-bg px-4">
        <div className="w-full max-w-sm rounded-lg border border-border bg-surface p-6 text-center">
          <p className="text-sm text-text-muted">
            Ce lien d'activation est incomplet. Utilisez le lien reçu par e-mail.
          </p>
          <Link to="/portail-prescripteur/login" className="mt-3 inline-block text-xs text-accent-light hover:underline">
      ...
      <div className="w-full max-w-sm">
        <div className="mb-8 flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            <h1 className="font-heading text-lg font-semibold text-text">Activer mon compte</h1>
            <p className="text-sm text-text-muted">Choisissez un mot de passe pour votre espace prescripteur.</p>
          </div>
        </div>
