...
export function PrescriberForgotPasswordPage() {
  const [email, setEmail] = useState("");
  const [sent, setSent] = useState(false);

  const mutation = useMutation({
    mutationFn: async () => {
      await prescriberApi.post("/portail-prescripteur/mot-de-passe-oublie", { email });
    },
    ...
    <div className="flex min-h-screen items-center justify-center bg-bg px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="space-y-4 rounded-lg border border-border bg-surface p-6 shadow-[var(--shadow-card)]">
          ...
          <Link to="/portail-prescripteur/login" className="block text-center text-xs text-accent-light hover:underline">
